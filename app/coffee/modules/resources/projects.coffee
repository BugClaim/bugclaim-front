...
resourceProvider = ($config, $repo, $http, $urls, $auth, $q, $translate) ->
    ...
    service.import = (file, statusUpdater) ->
        ...
        complete = (evt) =>
            ...
            if evt.target.getResponseHeader('Bugclaim-Info-Project-Is-Private')
                response.headers = {
                    isPrivate: evt.target.getResponseHeader('Bugclaim-Info-Project-Is-Private') == 'True',
                    memberships: parseInt(evt.target.getResponseHeader('Bugclaim-Info-Project-Memberships'))
                }
